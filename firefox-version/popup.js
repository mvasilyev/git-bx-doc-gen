...
Format your response in clean, readable markdown.`
  ...
  async function loadOllamaModels() {
    ...
    if (models && models.length > 0) {
      ...
      // Check if current config is for Ollama and restore the saved model
      const isOllama = config.baseUrl.includes('localhost:11434') || config.baseUrl.includes('127.0.0.1:11434');
      if (isOllama && config.model) {
        // Check if the saved model is available in Ollama
        const modelExists = models.some(m => m.name === config.model);
        if (modelExists) {
          // Set the saved Ollama model
          modelSelect.value = config.model;
        } else {
          // If saved model not found, set to first available Ollama model
          modelSelect.value = models[0].name;
        }
      } else if (config.model && !defaultOptions.includes(config.model)) {
        // Handle custom model for non-Ollama
  ...
  async function loadConfig() {
    try {
      ...
      if (isOllama) {
        // For Ollama, we'll set the model after loading available models
        // Don't set the model value yet, wait for loadOllamaModels
      } else {
        // For non-Ollama endpoints, set the model immediately
